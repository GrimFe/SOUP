# import Serpent2InputWriter as siw
#
# __all__ = [
#     "COMPOSITON_GU3_REFERENCE_FILE",
#     "GEOMETRY_GU3_REFERENCE_FILE",
#     "DEPLETION_GU3_REFERENCE_FILE",
#     "SIMULATION_GU3_REFERENCE_FILE",
#     "composition_gu3_16",
#     "geometry_gu3_16",
#     "depletion_gu3_16",
#     "simulation_gu3_cycle_16"
# ]
#
# COMPOSITON_GU3_REFERENCE_FILE = '/gu3_composition.reference'
# GEOMETRY_GU3_REFERENCE_FILE = '/gu3_geometry.reference'
# DEPLETION_GU3_REFERENCE_FILE = '/gu3_depletion.reference'
# SIMULATION_GU3_REFERENCE_FILE = '/gu3_simulation.reference'
#
# # --------------------------------- PARAMETERS ------------------------------------
# pin_radius = 0.4555
...
#     guide_tube = siw.Pin(name=2, radi=[(water, 0.62), (zircaloy, 0.69), (water.copy(), 0)])
#     sample_pin = siw.Pin(name=3, radi=[(fuel_sample, pin_radius), (helium, 0.465), (zircaloy, 0.5375), (water, 0)])
#
...
#                                    sample_fuel_region, water_around_sample, lattice_cell, out_of_lattice,
#                                    external_boundary],
#                             lattices=[lattice])
#     return geometry
#
#
# def depletion_gu3_16():
...
#
#
# def simulation_gu3_cycle_16(reset=False):
#     # --------------------------------- PARAMETERS ------------------------------------
#     pin_radius_ = 0.4555
#     cell_side_length_ = 0.715 * 2
#     extra_water_thickness_ = 0.055
#     n_side_pins_ = 15
#
#     # --------------------------------- COMPOSITION ------------------------------------
#     fuel_temperature_ = 1200
#     environment_temperature_ = 600
#     fuel_ = siw.Material(name='fuel_1',
#                          dens=-10.4,
#                          comment=siw.Comment("UO2 fuel with enrichment 4.1%"),
#                          representation=siw.MaterialRepresentation.from_material_composition(
#                              siw.MaterialComposition.from_za(
#                                  {92234: 9.90723e-6, 92235: 9.63007e-4, 92238: 2.22307e-2, 8016: 4.64201e-2,
#                                   8017: 1.85755e-5}), tmp=fuel_temperature_),
#                          burn=True,
#                          tmp=fuel_temperature_
#                          )
#     fuel_.divide(lvl=1, kind="radial", sub=(10, 0, pin_radius_))
#     fuel_sample_ = siw.Material(name='fuel_3',
#                                 dens=-10.4,
#                                 comment=siw.Comment("UO2 fuel with enrichment 4.1% - Fuel sample"),
#                                 representation=siw.MaterialRepresentation.from_material_composition(
#                                     siw.MaterialComposition.from_zam(
#                                         {922340: 9.90723e-6, 922350: 9.63007e-4, 922380: 2.22307e-2, 80160: 4.64201e-2,
#                                          80170: 1.85755e-5}), tmp=fuel_temperature_),
#                                 burn=True,
#                                 tmp=fuel_temperature_
#                                 )
#     fuel_sample_.divide(lvl=1, kind="radial", sub=(10, 0, pin_radius_))
#     helium_ = siw.Material(name='He_2',
#                            dens=1.23372e-4,
#                            comment=siw.Comment("Helium gap material"),
#                            representation=siw.MaterialRepresentation.from_material_composition(
#                                siw.MaterialComposition.from_nuclides({"He-3": 2.46744e-10, "He-4": 1.23372e-4}),
#                                tmp=environment_temperature_),
#                            tmp=environment_temperature_
#                            )
#     zircaloy_ = siw.Material(name='clad_3',
#                              dens=-6.55,
#                              tmp=environment_temperature_,
#                              comment=siw.Comment("Zircaloy cladding"),
#                              representation=siw.MaterialRepresentation.from_string(string="""    40090 2.18292E-02
...
#                                                                                    tmp=environment_temperature_)
#                              )
#     water_components_ = "H1, H2, O16, O17, B10, B11".split()
#     water_densities_ = [4.70654e-2, 5.41314e-6, 2.35265e-2, 8.94345e-6, 5.37619e-6, 1.96183e-5]
#     moderator_library_ = "lwe70"
#     hydrogen_za_ = 1001
#     water_ = siw.Material(name="water",
#                           dens=-0.7042,
#                           tmp=environment_temperature_,
#                           comment=siw.Comment("Borated water - 638 ppm B"),
#                           moder=[(moderator_library_, hydrogen_za_)],
#                           representation=siw.MaterialRepresentation.from_material_composition(
#                               siw.MaterialComposition.from_nuclides(dict(zip(water_components_, water_densities_))),
#                               tmp=environment_temperature_
#                           ))
#
#     # --------------------------------- GEOMETRY ------------------------------------
...
#     external_universe = siw.NestedUniverse(name=0)
#
#     fuel_pin = siw.Pin(name=1, radi=[(fuel_, pin_radius_), (helium_, 0.465), (zircaloy_, 0.5375), (water_, 0)])
#     guide_tube = siw.Pin(name=2, radi=[(water_, 0.62), (zircaloy_, 0.69), (water_.copy(), 0)])
#     sample_pin = siw.Pin(name=3, radi=[(fuel_sample, pin_radius), (helium, 0.465), (zircaloy, 0.5375), (water, 0)])
#
#     pin_delimiter = siw.Surface(name=100, parameters=[0, 0, cell_side_length_ / 2])
#     lattice_delimiter = siw.Surface(name=7, parameters=[0, 0, cell_side_length_ / 2 * n_side_pins_])
#     external_delimiter = siw.Surface(name=1, parameters=[0, 0, cell_side_length_ / 2 * n_side_pins_ +
#                                                          extra_water_thickness_])
#
#     fuel_region = siw.Cell(name=1,
#                            father=all_fuel,
#                            delimiters=siw.surface_complement([pin_delimiter]),
...
#                            parameters=[0, 0, n_side_pins_, n_side_pins_, 1.43],
#                            representation=siw.LatticeRepresentation.from_cartesian(shape=(n_side_pins_, n_side_pins_),
#                                                                                    filler=all_fuel,
#                                                                                    other=[
#                                                                                        (all_guides, control_rod_coordinates),
#                                                                                        (sample_universe, sample_position)]))
...
#     power_densities = [0.067167, 0.067833, 0.067833, 0.067833, 0.067833, 0.067833, 0.067833, 0.062500,
#                        0.062500, 0.062500, 0.062500, 0.062500, 0.062500, 0.058036, 0]
#     n = [siw.Normalization(value=p, material=fuel_sample_) for p in power_densities]
#
#     time_steps = [6, 23.54, 29.48, 29.48, 29.48, 29.48, 2.52, 29.26, 32, 32, 32, 32, 12.74, 16.8, 34.2]
#     t = [siw.Interval(span=t) for t in time_steps[:-1]] + [siw.Interval(span=time_steps[-1], span_type='dec')]
#
#     # --------------------------------- WRAPPERS --------------------------------------
#     material_composition = siw.Composition(
#         materials=[fuel_, fuel_sample_, helium_, water_, zircaloy_],
#         libraries={
#             "acelib": "/srv/sci/pack/nuclear-data/endfb_71/ace/xsdata.endfb71",
#             "bralib": "/srv/sci/pack/nuclear-data/endfb_71/bralib.endfb71",
#             "declib": "/srv/sci/pack/nuclear-data/endfb_71/decay/rdd.endfb71",
...
#     cycles = (175, 25)
#     simulation = siw.Simulation(geometry, depletion, material_composition, cycles=cycles, comments=comments)
#     simulation.seed = 1423124306
#     simulation.population = (100000, 238, 30)
#
#     if reset:
#         exitfile = COMPOSITON_GU3_REFERENCE_FILE
#         clear_file(exitfile)
#         material_composition.write(exitfile)
#
#         exitfile = GEOMETRY_GU3_REFERENCE_FILE
#         clear_file(exitfile)
#         geometry.write(exitfile)
#
#         exitfile = DEPLETION_GU3_REFERENCE_FILE
#         clear_file(exitfile)
#         depletion.write(exitfile)
#
#         exitfile = SIMULATION_GU3_REFERENCE_FILE
#         clear_file(exitfile)
